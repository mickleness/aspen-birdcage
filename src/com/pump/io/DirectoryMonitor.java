...
/**
 * This monitors a <code>java.io.File</code> directory and notifies its
 * listeners when files are added, removed or changed.
 */
public class DirectoryMonitor implements AutoCloseable {

	/**
	 * This listener is notified every time there new, deleted, or modified
	 * files to report.
	 * <p>
	 * Note that this is not guaranteed to capture all events. For example, if a
	 * file is created and deleted before this monitor polls the file system: it
	 * won't trigger any events.
	 */
	public interface Listener {
		public void listingChanged(File[] filesAdded, File[] filesDeleted,
				File[] filesChanged);
	}
		...
		public void run() {
			...
			if (added.size() > 0 || changed.size() > 0 || removed.size() > 0) {
				...
				synchronized (monitor) {
					listenerArray = monitor.listeners
							.toArray(new Listener[monitor.listeners.size()]);
				}
	 ...
	 * 
	 * @param file
	 *            the directory to monitor. This must already exist or this
	...
	public synchronized void addListener(Listener listener) {
		...
		if (listeners.size() == 1) {
			scheduledFuture = service.scheduleAtFixedRate(
					new ProcessFileRunnable(this), 0, pollingInterval,
	...
	/**
	 * This terminates the polling thread (executor) so we stop listening to the
	 * directory. Once this method is called this object cannot be reactivated.
	 */
	@Override
