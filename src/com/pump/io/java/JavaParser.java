...
public class JavaParser {
	 ...
	 * 
	 * @param reader
	 *            the reader to create {@link com.pump.io.Token} objects from.
	 ...
	 * @throws IOException
	 */
	public static void parse(Reader reader, boolean includeWhitespaceTokens,
			Receiver<Token> receiver) throws IOException {
		if (reader == null)
			...
			parseTokens: while (l.current() != CharacterIterator.DONE) {
				...
				if (ch == '/' && next == '*') {
					...
					readLiteral: while (l.current() != CharacterIterator.DONE
							&& error == null && ex == null) {
						// store at least 6 chars, because a unicode char is the
						// longest char we may parse: \u0123
					...
					// parse a word (any consecutive string letters/digits that
					// follow a letter):
				...
				readNumber: if (ch == '+' || ch == '-' || Character.isDigit(ch)
					...
					if (ch == '+' || ch == '-') {
						...
						if (l.peek(pos) == '.') {
							...
							if (l.peek(pos) == '.') {
								...
								if (clusterCtr == 0) {
									// we're not reading a number, we just had a
									// plus or minus sign.
									break readNumber;
								}
							}
						...
						if (clusterCtr == 0) {
							// how did we get here if we didn't have a cluster
							// of digits?
							throw new RuntimeException("Unexpected condition");
						}
						if (l.peek(pos) == '.') {
					...
					// whew. That was the hard(er) part. Now we just parse the
					// optional tail of that expression:
					Token newToken;
					...
					if (brackets != null) {
						if (bct.open) {
							...
							if (last != null && last.open
									&& last.bracketType.equals(bct.bracketType)) {
								last.setMatching(bct);
							} else {
								// signal that bracket matching is broken, and
	...
	public static Token[] parse(String expr, boolean includeWhitespaceTokens) {
		...
		try {
			JavaParser.parse(new StringReader(expr), includeWhitespaceTokens,
					receiver);
		} catch (IOException e) {
	 ...
	 * @throws IOException
	 */
	public static Token[] parse(Reader reader, boolean includeWhitespaceTokens)
			throws IOException {
		BasicReceiver<Token> receiver = new BasicReceiver<>();
	 ...
	 */
	public static Token[][] parseLines(Reader reader,
			boolean includeWhitespaceTokens) throws IOException {
