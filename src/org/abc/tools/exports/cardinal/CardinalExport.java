...
public class CardinalExport extends ToolJavaSource {
	...
	protected void run() throws Exception {
		try (OutputStream out = getResultHandler().getOutputStream()) {
			...
			html = html.replace(CardinalExport.keywordReplacementKey,
					createJavascript());
	...
	private int getRecordCount(BeanTablePath btp) throws Exception {
		...
		String sql = "SELECT COUNT(" + oidPath.getDatabaseName() + ") FROM "
				+ btp.getDatabaseName() + ";";
	...
	/**
	 * Create a block of javascript that begins with "function
	 * initializeTableInfo() {"
	 */
	private String createJavascript() throws Exception {
		boolean includeTableRowCount = Boolean.parseBoolean(
				String.valueOf(getParameter(PARAM_INCLUDE_TABLE_ROW_COUNT)));
		...
		for (int a = 0; a < allColumns.size(); a++) {
			...
			String longName = "";
			if (fdd != null) {
				shortName = field.getUserShortName() == null ? ""
						: field.getUserShortName().replace("'", "");
				longName = field.getUserLongName() == null ? ""
						: field.getUserLongName().replace("'", "");
			}
		...
		for (int a = 0; a < allRelationships.size(); a++) {
			...
			String relationshipOid = (String) BeanTablePath.class
					.getMethod("getRelationshipOid").invoke(btp);
