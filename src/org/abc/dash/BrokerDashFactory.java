...
public class BrokerDashFactory {
	...
	/**
	 * A key in our cache is a combination of an Operator (the "WHERE" clause) 
	 * and the comparator (the "ORDER-BY" clause)
	 */
	private static class Key {
		public final Operator operator;
		public final OrderByComparator orderBy;

		public Key(Operator operator, OrderByComparator orderBy) {
	...
	static class DashInvocationHandler implements InvocationHandler {
		...
		protected Object invokeCached(Object proxy, Method method, Object[] args)
			...
			if (method_getIteratorByQuery.equals(method)
				...
				Cache<Key, List<String>> cache = cacheByBeanType.get(beanType);
				if (cache != null)
					cache.clear();
		...
		protected QueryIterator createdCachedIterator(final BeanQuery beanQuery) {
			...
			final Key primaryCacheKey = new Key(operator, orderBy);
			...
			List<String> beanOids = cache.get(primaryCacheKey);
			if (beanOids != null)
			...
			return new BeanIteratorWrapper(cache, primaryCacheKey, iter, 1000);
		}
	}
